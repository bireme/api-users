...
def update_api_gateway(profile, request):
    url = f"{settings.APISIX_ADMIN_URL}/apisix/admin/consumers"

    payload = {
        "username": profile.user.username,
        "desc": f"{profile.licence_id} / {profile.licence_notes}",
        "plugins": { "key-auth": {
                "disable": True,
    ...
    if not response.ok:
        ...
        messages.success(request, "O token de API do usuário foi atualizado com sucesso.")
...
class Profile(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.licence_id and self.api_token:
            try:
                update_api_gateway(self, request)
